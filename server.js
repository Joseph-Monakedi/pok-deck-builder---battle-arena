...
async function startServer() {
  ...
  io.on('connection', (socket) => {
        ...
        if (p1Socket && p2Socket) {
          ...
          const battleState = {
            ...
            players: {
              [p1Id]: {
                id: p1Id,
                deck: p1Socket.data.deck.map((p) => ({ ...p, currentHp: p.stats.hp, maxHp: p.stats.hp, isFainted: false })),
              ...
              [p2Id]: {
                id: p2Id,
                deck: p2Socket.data.deck.map((p) => ({ ...p, currentHp: p.stats.hp, maxHp: p.stats.hp, isFainted: false })),
                activePokemonIndex: 0,
                isReady: true,
                isAuto: false,
              },
    ...
    socket.on('join_cpu_battle', (deck) => {
      ...
      const cpuDeck = deck.map((p) => ({ ...p, currentHp: p.stats.hp, maxHp: p.stats.hp, isFainted: false }));
      ...
      const battleState = {
        ...
        players: {
          [socket.id]: {
            id: socket.id,
            deck: deck.map((p) => ({ ...p, currentHp: p.stats.hp, maxHp: p.stats.hp, isFainted: false })),
            activePokemonIndex: 0,
            isReady: true,
            isAuto: false,
          },
    ...
    function checkAndTriggerNextTurn(state, roomId) {
    ...
    function processAttack(state, attackerId, opponentId, move) {
      ...
      opponentPoke.types.forEach((defType) => {
      ...
      if (opponentPoke.currentHp <= 0) {
        ...
        const allFainted = opponent.deck.every(() => p.isFainted);
        if (allFainted) {
          state.status = 'finished';
          state.winner = attackerId;
          state.log.unshift(`${attackerId === 'cpu' ? 'CPU' : 'Player'} wins the battle!`);
        } else {
          const nextIndex = opponent.deck.findIndex((p) => !p.isFainted);
